...
function compute_branching_result_with_region(cache::RegionCache, problem::TNProblem, var_id::Int, measure::AbstractMeasure, set_cover_solver::AbstractSetCoverSolver)
    region, cached_configs = get_region_data!(cache, problem, var_id)

    # Filter configs that are compatible with current doms
    feasible_configs = filter_feasible_configs(problem, region, cached_configs, measure)
    isempty(feasible_configs) && return nothing, region.vars, region, 0
    ...
    isempty(unfixed_vars) && return nothing, region.vars, region, length(feasible_configs)
    ...
    # Support size is the number of unique projected feasible configurations
    support_size = length(projected)

    # Build branching table from filtered configs
    table = BranchingTable(length(unfixed_vars), [[c] for c in projected])
    # Compute optimal branching rule
    result = OptimalBranchingCore.optimal_branching_rule(table, unfixed_vars, problem, measure, set_cover_solver)
    return result, unfixed_vars, region, support_size
end
