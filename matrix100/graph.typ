...
#let show-grid-graph(m, n, filling:1, gridsize: 1, unitdisk:1.1, radius: 0.2, a:(1, 0), b:(0, 1), seed:42) = {
  import draw: *
  let locations = grid-graph-locations(m, n, filling:filling, gridsize: gridsize, a:a, b:b, seed:seed)
  show-udg-graph(locations, unit:unitdisk * gridsize, radius:radius)
}

#let spring-layout(n, edges, optimal_distance:2, seed:42, maxiter:100, alpha0:2.0) = {
  let locs = ()
  let rng = gen-rng(seed)
  for i in range(n) {
    let locs_i
