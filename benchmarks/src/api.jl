...
function _extract_bi_stats(stats)
    # BranchingStats field names: children_explored, branching_nodes, reduction_nodes, etc.
    branches = hasfield(typeof(stats), :children_explored) ? stats.children_explored : 0
    conflicts = hasfield(typeof(stats), :unsat_leaves) ? stats.unsat_leaves : 0
    gamma_one = hasfield(typeof(stats), :reduction_nodes) ? stats.reduction_nodes : 0
    ...
    gamma_one_vars = hasfield(typeof(stats), :reduction_direct_assignments) ?
        stats.reduction_direct_assignments + stats.reduction_implied_assignments : 0
    branch_vars = hasfield(typeof(stats), :branch_decision_assignments) ?
        stats.branch_decision_assignments + stats.branch_implied_assignments : 0
    return (branches, conflicts, gamma_one, avg_vars, gamma_one_vars, branch_vars)
end
...
function _to_result(raw, solver_name::String, elapsed::Float64)
    ...
    if hasfield(typeof(raw), :found) && hasfield(typeof(raw), :stats)
        branches, conflicts, gamma_one, avg_vars, γ1_vars, br_vars = _extract_bi_stats(raw.stats)
        return SolveResult(raw.found ? SAT : UNSAT, raw.solution, elapsed, branches, conflicts, gamma_one, avg_vars, γ1_vars, br_vars, solver_name)
    end

    # BooleanInferenceSolver: (is_sat::Bool, stats)
    if raw isa Tuple && length(raw) == 2 && raw[1] isa Bool
        branches, conflicts, gamma_one, avg_vars, γ1_vars, br_vars = _extract_bi_stats(raw[2])
        return SolveResult(raw[1] ? SAT : UNSAT, nothing, elapsed, branches, conflicts, gamma_one, avg_vars, γ1_vars, br_vars, solver_name)
    end
    ...
    if raw isa Tuple && length(raw) == 3
        ...
        if !isnothing(p) && !isnothing(q)
            branches, conflicts, gamma_one, avg_vars, γ1_vars, br_vars = isnothing(stats) ? (0, 0, 0, 0.0, 0, 0) : _extract_bi_stats(stats)
            return SolveResult(SAT, (p=p, q=q), elapsed, branches, conflicts, gamma_one, avg_vars, γ1_vars, br_vars, solver_name)
        end
        return SolveResult(UNSAT, nothing, elapsed, 0, 0, 0, 0.0, 0, 0, solver_name)
    end
    ...
    if raw isa CNFSolverResult
        ...
        conflicts = something(raw.conflicts, 0)
        return SolveResult(status, raw.model, elapsed, something(raw.decisions, 0), conflicts, 0, 0.0, 0, 0, solver_name)
    end

    SolveResult(UNKNOWN, raw, elapsed, 0, 0, 0, 0.0, 0, 0, solver_name)
end
