...
@testset "solve_sat_with_assignments" begin
    ...
    @test stats.branch_points >= 0
    @test stats.branches_explored >= 0
...
@testset "solve_factoring" begin
    ...
    @test stats.branch_points >= 0
    @test stats.branches_explored >= 0
    println("Factoring stats: branch_points=$(stats.branch_points), explored=$(stats.branches_explored)")
end
...
@testset "branching_statistics" begin
    ...
    @test initial_stats.branch_points == 0
    @test initial_stats.branches_explored == 0
    ...
    @test result.stats.branch_points >= 0
    @test result.stats.branches_explored >= 0
    ...
    @test reset_stats.branch_points == 0
    @test reset_stats.branches_explored == 0
end
