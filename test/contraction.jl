...
using BooleanInference: extract_inner_configs, combine_configs, handle_no_boundary_case_unfixed
using BooleanInference: clear_all_region_caches!
...
# These test sets are commented out because the functions they test
# (separate_fixed_free_boundary, construct_boundary_config, construct_inner_config)
# are no longer in the codebase - they appear to have been refactored.
# The functionality is now handled internally by other functions.
...
@testset "extract_inner_configs" begin
    ...
    # extract_inner_configs now returns Vector{UInt64}, not Vector{Bool}
    contracted_1d = reshape([T1, T0], 2)
    configs = extract_inner_configs(contracted_1d, 1)
    @test length(configs) == 1
    @test configs[1] == UInt64(0)  # 0 in binary = [false]
    
    contracted_2d = reshape([T1, T0, T1, T1], 2, 2)
    configs = extract_inner_configs(contracted_2d, 2)
    @test length(configs) == 3
    # Julia uses column-major order, so:
    # Linear index 1 -> [1,1] -> T1 -> config 0 (0b00 = [false, false])
    # Linear index 2 -> [2,1] -> T0 -> config 1 (0b01 = [false, true]) - NOT satisfied, so not in configs
    # Linear index 3 -> [1,2] -> T1 -> config 2 (0b10 = [true, false])
    # Linear index 4 -> [2,2] -> T1 -> config 3 (0b11 = [true, true])
    @test UInt64(0) in configs  # [false, false]
    @test !(UInt64(1) in configs)  # [false, true] - NOT satisfied
    @test UInt64(2) in configs  # [true, false]
    @test UInt64(3) in configs  # [true, true]
...
# construct_inner_config is no longer in the codebase - it appears to have been refactored.
...
@testset "combine_configs" begin
    # combine_configs now takes (boundary_bits::UInt64, inner_configs::Vector{UInt64}, n_boundary::Int)
    # and returns Vector{UInt64}
    # Note: bit representation uses LSB first, so 0b01 = [true, false], 0b10 = [false, true]
    boundary_bits = UInt64(0b01)  # [true, false] = 1 in binary (bit0=1, bit1=0)
    inner_configs = [UInt64(0b01), UInt64(0b10)]  # [true, false] and [false, true]
    n_boundary = 2
    
    full_configs = combine_configs(boundary_bits, inner_configs, n_boundary)
    @test length(full_configs) == 2
    # boundary=01 (1), inner=01 (1) -> full = 01 | (01 << 2) = 1 | 4 = 5 (0b101 = [true, false, true, false])
    # boundary=01 (1), inner=10 (2) -> full = 01 | (10 << 2) = 1 | 8 = 9 (0b1001 = [true, false, false, true])
    @test UInt64(5) in full_configs  # [true, false, true, false]
    @test UInt64(9) in full_configs  # [true, false, false, true]
    
    boundary_bits = UInt64(0b1)  # [true]
    inner_configs = [UInt64(0b0)]  # [false]
    n_boundary = 1
    
    full_configs = combine_configs(boundary_bits, inner_configs, n_boundary)
    @test length(full_configs) == 1
    @test full_configs[1] == UInt64(1)  # [true, false]
    
    boundary_bits = UInt64(0b0)  # []
    inner_configs = [UInt64(0b11), UInt64(0b00)]  # [true, true] and [false, false]
    n_boundary = 0
    
    full_configs = combine_configs(boundary_bits, inner_configs, n_boundary)
    @test length(full_configs) == 2
    @test UInt64(3) in full_configs  # [true, true]
    @test UInt64(0) in full_configs  # [false, false]
end
